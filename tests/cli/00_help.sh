...
"$WF" help whoami 2>&1 | grep -q "usage: wf whoami"
"$WF" help whoami 2>&1 | grep -q "Requires an active session from 'wf exec' or 'wf env export'"
...
"$WF" whoami --help 2>&1 | grep -q "usage: wf whoami"
"$WF" whoami --help 2>&1 | grep -q "current authenticated wf session username"
