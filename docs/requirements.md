...
- Top-level commands (`login`, `logout`, `version`, `user`, `issue`, `domain`) and entity subcommands under `user`, `issue`, and `domain` may be abbreviated to a unique prefix (e.g. `wf logi`, `wf ver`, `wf us pas`, `wf i cr`, `wf dom cur`).
